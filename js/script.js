...
const fecha_objetivo = new Date("November 7, 2026 21:00:00").getTime();
...
const NUMERO_WHATSAPP = '541128172864';
...
let restriccionSeleccionada = null;
...
const botonesRestriccion = document.querySelectorAll('.restriccion__button');
...
// Lógica para botones de restricción (Selección única)
botonesRestriccion.forEach(boton => {
    boton.addEventListener('click', (e) => {
        e.preventDefault();
        // Quitar selección de todos
        botonesRestriccion.forEach(b => b.classList.remove('seleccionado'));
        // Seleccionar el actual
        boton.classList.add('seleccionado');
        // Guardar valor
        restriccionSeleccionada = boton.innerText;
    });
});
...
function enviarConfirmacion(numeroDestino) {
    ...
    // Validar restricción alimenticia
    if (!restriccionSeleccionada) {
        alert('Por favor, selecciona una restricción alimenticia (o "Ninguna").');
        return;
    }
    ...
    // Crear mensaje
    const mensaje = `Hola confirmo que ${asistenciaTexto}, mi nombre es ${nombre} y mi restriccion alimenticia es ${restriccionSeleccionada}`;
...
// Event listener para confirmar a Alma
if (confirmarBtn2) {
